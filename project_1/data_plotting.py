...
def create_and_save_plot(data, ticker, period_start, period_end, std_close, filename=None):
    ...
    if 'Date' not in data:
        ...
        if pd.api.types.is_datetime64_any_dtype(data.index):
            ...
            plt.fill_between(dates, data['Close'].mean() - std_close, data['Close'].mean() + std_close,
                             color='gray',
        ...
        plt.fill_between(data['Date'], data['Close'].mean() - std_close, data['Close'].mean() + std_close, color='gray',
                         alpha=0.5, label='Standard Deviation')
